...
public class UDPMessageSender extends Thread implements LoggingServer {
    ...
    public void run() {
        while (!this.isInterrupted()) {
            try {
                if(this.logger == null){
                    this.logger = initializeLogging(UDPMessageSender.class.getSimpleName() + "-on-server-with-udpPort-" + this.serverUdpPort);
                }
        ...
        this.logger.log(Level.SEVERE,"Exiting UDPMessageSender.run() on port " + serverUdpPort);
    }
}